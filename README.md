...
| 이름      | 담당 영역             | 구현 내용                                                                                                                                           |
|---------|-------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
| **허지웅** | 유저 관리, 보안           | - Security 구성 (form login, oauth2.0 - GOOGLE, KAKAO) <br> - JWT 구성 <br> - 사용자 도메인 구현 <br> - JWT 블렉리스트, Redis 복구 로직  |
| **김승찬** | 알림, 메세지     | - 실시간 알림기능 구현  <br> - DM기능 구현 <br>- 로깅작업       |
| **변희재** | 인프라, 의상 추천 알고리즘     | - CI/CD 구축 <br> - 그라파나, 프로메테우스 도입  <br> - 자체 의상 추천 알고리즘 구현  <br> - LLM 기반 의상 추천 구현 <br> - 팔로우 API 구현           |
...
- ### 메세지 괸리
- `GET /api/direct-messages` - 메세지 목록
- `MessageMapping /api/direct-messages/direct-messages_send` - 메세지 전송
